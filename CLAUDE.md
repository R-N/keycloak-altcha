...
- `LoginAltchaAuthenticator` extends `UsernamePasswordForm`: it lets the parent render/validate the credentials form and injects the challenge in `createLoginForm` (initial + password-fail re-renders); its own captcha-failure re-renders go through `challengeWithError`. Captcha failures use `INVALID_CREDENTIALS` on purpose — avoids a bot-facing oracle distinguishing captcha vs password.
