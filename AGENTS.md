...
- `LoginAltchaAuthenticator` extends `UsernamePasswordForm`: parent renders/validates credentials, challenge injected in `createLoginForm` (initial + password-fail re-renders); captcha-failure re-renders go through `challengeWithError`. Captcha failures use `INVALID_CREDENTIALS` on purpose — avoids a bot-facing oracle.
